  ...
  "suggestions": [
    ...
    try {
      const response = await this.makeZhipuRequest(
        messages,
        1200,
        this.calculateAdaptiveTimeout(prompt.length, 1200, options.maxTimeoutMs, 25000),
        2
      );
...
请严格按照JSON格式返回评估结果。`;
  ...
  private parseAIResponse(content: string): any {
    try {
      ...
      if (jsonContent) {
        ...
        // 验证必需的字段
        if (!normalized.scores || !normalized.analysis || !normalized.summary) {
          logger.warn("AI响应缺少必需字段", {
            hasScores: !!normalized.scores,
  ...
  private normalizeAIResponse(raw: any): any {
    if (raw?.scores) {
      return raw;
    }
